...
class TestGrpoFastVLLM(TestGrpoFastBase):
    ...
    def test_batch_splitting_and_engine_configurations(self, vllm_num_engines: int, num_unique_prompts_rollout: int):
        ...
        for _ in range(num_unique_prompts_rollout):
            ...
            q, gt, d, raw_q, _ = pending_queries_map.pop(dataset_index)
...
class GrpoIntegrationTests(TestGrpoFastBase):
    ...
    def test_out_of_order_processing(self):
        ...
        (combined_result, batch, reward_metrics, batch_stats, prompt_pass_entries) = (
        ...
        self.assertEqual(len(pending_queries_map), 0)
        self.assertEqual(len(prompt_pass_entries), num_prompts)
        first_entry = sorted(prompt_pass_entries, key=lambda entry: entry.dataset_index)[0]
        self.assertEqual(first_entry.training_step, 1)
        self.assertEqual(first_entry.epoch_number, 0)
        self.assertAlmostEqual(first_entry.pass_rate, 0.375, places=3)
        self.assertEqual(first_entry.prompt_id, f"{first_entry.dataset_name}::{first_entry.dataset_index}")
...
class TestShufflingIterator(unittest.TestCase):
    ...
    def _make_entry(self, dataset_index: int, epoch: int, pass_rate: float) -> grpo_fast.PromptPassEntry:
        prompt_id = f"dataset::{dataset_index}"
        return grpo_fast.PromptPassEntry(
            prompt_id=prompt_id,
    ...
    def _advance_epoch(self, iterator: grpo_fast.ShufflingIterator) -> None:
        iterator.index = iterator.effective_size
        next(iterator)
    ...
    def test_curriculum_orders_by_pass_rate_and_limits_zero_prompts(self):
        data = np.arange(6)
        iterator = grpo_fast.ShufflingIterator(
            data, batch_size=1, seed=0, enable_prompt_pass_curriculum=True, zero_pass_curriculum_fraction=0.25
        )

        entries = [
            self._make_entry(0, 0, 0.2),
            self._make_entry(1, 0, 0.9),
            self._make_entry(2, 0, 0.0),
            self._make_entry(3, 0, 0.5),
            self._make_entry(4, 0, 0.75),
            self._make_entry(5, 0, 0.0),
        ]
        iterator.update_prompt_pass_entries(entries)

        self._advance_epoch(iterator)

        ordered_indices = iterator.data.tolist()
        self.assertEqual(ordered_indices, [1, 4, 3, 0, 2])
        self.assertEqual(iterator.effective_size, len(ordered_indices))

    def test_curriculum_center_sort_prioritizes_mid_pass_rates(self):
        data = np.arange(7)
        iterator = grpo_fast.ShufflingIterator(
            data,
            batch_size=1,
            ...
            zero_pass_curriculum_fraction=0.0,
            prompt_pass_curriculum_05sort=True,
        )

        entries = [
            self._make_entry(0, 0, 0.9),
            self._make_entry(1, 0, 0.1),
            self._make_entry(2, 0, 0.5),
            self._make_entry(3, 0, 0.75),
            self._make_entry(4, 0, 0.25),
            self._make_entry(5, 0, 0.625),
            self._make_entry(6, 0, 0.375),
        ]
        iterator.update_prompt_pass_entries(entries)
        ...
        ordered_indices = iterator.data.tolist()
        self.assertEqual(ordered_indices, [2, 6, 5, 4, 3, 1, 0])

    def test_zero_pass_backlog_prioritizes_oldest(self):
        data = np.arange(6)
        iterator = grpo_fast.ShufflingIterator(
            data, batch_size=1, seed=0, enable_prompt_pass_curriculum=True, zero_pass_curriculum_fraction=0.25
        )
        ...
        iterator.update_prompt_pass_entries(epoch0_entries)
        self._advance_epoch(iterator)

        epoch1_entries = [
            self._make_entry(0, 1, 0.4),
            self._make_entry(1, 1, 0.95),
            self._make_entry(2, 1, 0.0),  # Remains zero
            self._make_entry(3, 1, 0.8),
            self._make_entry(4, 1, 0.7),
        ]
        iterator.update_prompt_pass_entries(epoch1_entries)
        self._advance_epoch(iterator)

        ordered_indices = iterator.data.tolist()
        self.assertIn(5, ordered_indices, "Oldest zero-pass prompt should be scheduled first")
        self.assertNotIn(2, ordered_indices, "New zero-pass prompt should wait if quota exhausted")

    def test_progress_fraction_triggers_reordering_when_replay_enabled(self):
        data = np.arange(4)
        iterator = grpo_fast.ShufflingIterator(
            data,
            batch_size=1,
            seed=0,
            enable_prompt_pass_curriculum=True,
            zero_pass_curriculum_fraction=0.25,
        ...
        self._advance_epoch(iterator)
        # Epoch completion should be queued until cumulative progress crosses 1.0.
        self.assertEqual(len(iterator._pending_epoch_completions), 1)
        self.assertEqual(iterator._last_reordered_progress_unit, 0)

        iterator.update_progress_fraction(1.0)
        iterator._maybe_prepare_epochs()

        expected_order = [1, 3, 0, 2]
        self.assertEqual(iterator.data.tolist(), expected_order)
        self.assertEqual(iterator._last_reordered_progress_unit, 1)
        self.assertFalse(iterator._pending_epoch_completions)

    def _make_replay_iterator(
        self,
        prompts_per_step: int = 4,
        replay_fraction: float = 0.5,
        cooldown_steps: int = 3,
        max_reuse_time: int | None = None,
    ) -> grpo_fast.ShufflingIterator:
        data = np.arange(8)
        iterator_kwargs = {
            "data": data,
            "batch_size": 1,
            ...
            "enable_prompt_replay": True,
            "prompt_replay_fraction": replay_fraction,
            "prompt_replay_cooldown_steps": cooldown_steps,
        }
        if max_reuse_time is not None:
            iterator_kwargs["prompt_replay_max_reuse_time"] = max_reuse_time
        return grpo_fast.ShufflingIterator(**iterator_kwargs)

    def test_replay_fraction_and_priority(self):
        iterator = self._make_replay_iterator(prompts_per_step=4)
        entries = [
            grpo_fast.PromptPassEntry("d::0", "d", 0, 0, 1, 0.2),
            grpo_fast.PromptPassEntry("d::1", "d", 1, 0, 1, 0.95),
            grpo_fast.PromptPassEntry("d::2", "d", 2, 0, 1, 0.4),
            grpo_fast.PromptPassEntry("d::3", "d", 3, 0, 1, 0.1),
            grpo_fast.PromptPassEntry("d::4", "d", 4, 0, 1, 0.9),
        ]
        iterator.update_prompt_pass_entries(entries)

        selections = [iterator.next_for_step(10) for _ in range(4)]
        reused = [idx for idx, meta in selections if meta.was_reused]
        self.assertEqual(len(reused), 2)
        self.assertSetEqual(set(reused), {1, 4}, "Top pass-rate prompts should be replayed first")

    def test_replay_cooldown_prevents_early_reuse(self):
        iterator = self._make_replay_iterator(prompts_per_step=4, cooldown_steps=3)
        entries = [
            grpo_fast.PromptPassEntry("d::0", "d", 0, 0, 1, 0.9),
            grpo_fast.PromptPassEntry("d::1", "d", 1, 0, 1, 0.85),
            grpo_fast.PromptPassEntry("d::2", "d", 2, 0, 1, 0.4),
            grpo_fast.PromptPassEntry("d::3", "d", 3, 0, 1, 0.3),
        ]
        iterator.update_prompt_pass_entries(entries)

        first_step = [iterator.next_for_step(5) for _ in range(4)]
        reused = [idx for idx, meta in first_step if meta.was_reused]
        self.assertTrue(reused)

        completion_entries = [
            grpo_fast.PromptPassEntry(
                prompt_id=f"d::{idx}",
                dataset_name="d",
                dataset_index=idx,
                epoch_number=0,
        ...
        iterator.update_prompt_pass_entries(completion_entries)

        early_step = [iterator.next_for_step(6) for _ in range(4)]
        self.assertFalse(any(meta.was_reused for _, meta in early_step), "Cooldown should block replay")

        cooled_step = [iterator.next_for_step(9) for _ in range(4)]
        self.assertTrue(any(meta.was_reused for _, meta in cooled_step), "Replay should resume after cooldown")

    def test_replay_entries_respect_max_reuse_limit(self):
        entries = [
            grpo_fast.PromptPassEntry("d::0", "d", 0, 0, 10, 0.95),
            grpo_fast.PromptPassEntry("d::1", "d", 1, 0, 10, 0.9),
        ]

        iterator = self._make_replay_iterator(
            prompts_per_step=2, replay_fraction=0.5, cooldown_steps=0, max_reuse_time=1
        )
        iterator.update_prompt_pass_entries(entries)

        first_step = [iterator.next_for_step(11) for _ in range(2)]
        reused = [idx for idx, meta in first_step if meta.was_reused]
        self.assertTrue(reused, "Prompts should be replayed until they hit the limit")
        ...
        second_step = [iterator.next_for_step(12) for _ in range(2)]
        self.assertFalse(
            any(idx in reused and meta.was_reused for idx, meta in second_step),
            "Prompts should stop replaying after reaching the reuse limit",
        )

    def test_replay_tie_breaks_by_reuse_count(self):
        iterator = self._make_replay_iterator(prompts_per_step=2, replay_fraction=0.5)
        entries = [
            grpo_fast.PromptPassEntry("d::0", "d", 0, 0, 1, 0.9),
            grpo_fast.PromptPassEntry("d::1", "d", 1, 0, 1, 0.9),
        ]
        iterator.update_prompt_pass_entries(entries)

        first = iterator.next_for_step(3)
        iterator.update_prompt_pass_entries(
            [
                grpo_fast.PromptPassEntry(
                    prompt_id=f"d::{first[0]}",
                    dataset_name="d",
                    dataset_index=first[0],
                    epoch_number=0,
                    training_step=3,
                    pass_rate=0.9,
                    was_reused=True,
        ...
        second = iterator.next_for_step(6)
        self.assertNotEqual(first[0], second[0], "Prompt with fewer reuses should be prioritized on ties")

    def test_pending_queries_map_preserves_metadata(self):
        pqm = grpo_fast.PendingQueriesMap()
        metadata = grpo_fast.PromptReplayMetadata(
            was_reused=True, reuse_count=2, scheduled_step=7, cooldown_ready_step=12
        )
        pqm.insert(0, [1, 2], [3], "dataset", "raw", metadata)
        _, _, _, _, popped = pqm.pop(0)
        self.assertTrue(popped.was_reused)
        self.assertEqual(popped.reuse_count, 2)
        self.assertEqual(popped.cooldown_ready_step, 12)
